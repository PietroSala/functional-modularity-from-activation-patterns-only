...
def main():
    ...
    space_size = 4
    mnist_dp = FashionMnistDataProvider(autoencoder=False, device='cuda', batch_size=32, cnn=True, num_kernels=17)
    mnist_space = ModularSpace('fashion_mnist_cnn', mnist_dp.model, space_size, device='cuda', useActivations=False, useLayers=True)
    minst_stats = ModularStats(mnist_space)
    ...
    if train:
        step_train(mnist_dp, mnist_space, epochs=10)


        minst_stats.metric_count(True)
        minst_stats.metric_covariance(True)
        step_train(mnist_dp, mnist_space, epochs=1)
        minst_stats.show_metric_count()
        minst_stats.show_metric_covariance()
    ...
    if space: 
        step_space(mnist_dp, mnist_space, epochs=1)
        vis.play()
    
    if data: 
        step_data(mnist_dp, mnist_space)
